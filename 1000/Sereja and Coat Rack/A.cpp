...
void solve(){
    ll n, d; cin>>n>>d;
    vll preciosPorGancho(n);
    rep(i, 0, n) cin>>preciosPorGancho[i];
    ll m; cin>>m;
    ll dineroGanado = 0, dineroPerdido = 0;

    sort(preciosPorGancho.begin(), preciosPorGancho.end());

    rep(i, 0, m) {
        if(i < n) dineroGanado+=preciosPorGancho[i];
        else break;
    }

    if(m > n) dineroPerdido = (abs(n-m) * d);

    cout<<dineroGanado - dineroPerdido<<el;
}