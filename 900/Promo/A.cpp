...
void solve(){
    ll n, q; cin>>n>>q;
    vll preciosProductos(n), prefixSum(n);
    rep(i, 0, n) cin>>preciosProductos[i];
    sort(ALL(preciosProductos));

    prefixSum[0] = preciosProductos[0];
    rep(i, 1, n) prefixSum[i] = prefixSum[i-1] + preciosProductos[i];

    rep(i, 0, q) {
        ll x, y; cin>>x>>y;
        
        if(x == n) cout<<prefixSum[y-1]<<el;
        else cout<<prefixSum[n - x - 1 + y] - prefixSum[n - x - 1]<<el;
    }   
}